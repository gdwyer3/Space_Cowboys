...
public abstract class EncounterController {
    protected Player player = ChooseLevelScreenController.newPlayerTest;
    protected int regiontoGo; // region player is trying to get to
    protected ArrayList<Button> regbuttons;
    protected ArrayList<Button> infoButtons;
    protected Object[] regionInfo;
    protected Set<Integer> prevVisit;
    protected double lastAdded;
    protected Region startRegion;
    @FXML
    protected Button endEncounter = new Button("End Encounter");
    ...
    protected void goToTravelLog(int region) { // return to specified region
        //close encounter window/ screen
        //playerInfo.getChildren().addAll(endEncounter);
        endEncounter.setOnMouseClicked(event -> {
            player.playerShip.decreaseFuelAfterTravel(); // decrease fuel by one
            // bc we want anchorpane not vbox
            Stage stage = (Stage) endEncounter.getScene().getWindow().getScene().getWindow();
            FXMLLoader loader =
            ...
            nextController.playerStart = region; // tells current region
    ...
    // specific encounters will implements with additional values to set
    protected abstract void setValues(Player player, int lastVisit, int regiontoGo);
}
